// RUN: %single-file-test insertsort_main
// END.
...
/*
  ...
  Changes: a brief summary of major functional changes (not formatting)
	2022-06-02: Randomly initialize array
				No longer ignore the first element in the sort
...
unsigned int rand() {
  return random_integer() % 100000;
}

void insertsort_init()
{
  unsigned int a[ 11 ] = {rand(), rand(), rand(), rand(), rand(), rand(), rand(), rand(), rand(), rand(), rand()};
