// RUN: %single-file-test matrix1_main false
// END.
...
void matrix1_pin_down( int A[  ], int B[  ], int C[  ], int );
void matrix1_init( int );
...
void matrix1_pin_down( int A[  ], int B[  ], int C[  ], int seed )
{
  int i;
  volatile int x = 1 + seed;
...
void matrix1_init( int seed )
{
  matrix1_pin_down( &matrix1_A[ 0 ], &matrix1_B[ 0 ], &matrix1_C[ 0 ], seed );
}
