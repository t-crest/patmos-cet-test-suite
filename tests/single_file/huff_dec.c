// RUN: %single-file-test huff_dec_main false
// END.
/*
...
void huff_dec_init( int seed )
{
  huff_dec_input_pos = 0;
  huff_dec_output_pos = 0;
  
  if(seed != 0) {
    for(int i = 0; i<huff_dec_encoded_len; i++) {
	  huff_dec_encoded[ i ] = (char) random_integer();
	}
  }
}
